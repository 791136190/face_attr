...
def do_rmfd():
    FaceDet = face_det.FaceDet(conf_threshold=0.45) # 0.3->134, 0.4->107, 0.45->101, 0.5->96, 0.7->83

    root_path = "/home/hu/work/CV/dataset/rmfd/Real-World-Masked-Face-Dataset/"
    
    image_save_root = "/home/hu/work/CV/dataset/rmfd/images/"
    if not os.path.exists(image_save_root):
        os.makedirs(image_save_root)

    fw = open("dataset/rmfd_list_mix.txt", 'w')
    ...
    label_list = ['RMFD_part_2/', 'RMFD_part_3/', 'RWMFD_part_1/']
    for part in label_list:

        lines = get_all_list(root_path + part)
        for line in lines:
            line = line.strip()   # /home/hu/work/CV/dataset/rmfd/Real-World-Masked-Face-Dataset/RWMFD_part_1/0000/0000.jpg

            info = 1# if 'imfd/' == part else 1

            cur_image_path = line # /home/hu/work/CV/dataset/rmfd/Real-World-Masked-Face-Dataset/RWMFD_part_1/0000/0000.jpg
            ...
            if cur_index % 10 == 0:
                print("do rmfd %d in %d, save_index:%d" % (cur_index, len(lines), save_index))
...
def do_celeba(start_index):
    ...
    fw = open("dataset/rmfd_list.txt", 'a+')
...
def del_noface():
    image_save_root = "/home/hu/work/CV/dataset/rmfd/images/"
    path_list = get_all_list(image_save_root)

    with open("dataset/rmfd_list_mix.txt", 'r') as fw:
        _list = fw.readlines()

    fw = open("dataset/rmfd_list.txt", 'w')
    for l in _list:
        file_name = l.split('|')[0]
        if file_name in path_list:
            fw.write(l)
        else:
            print(l)
    fw.close()
...
if __name__ == '__main__':
    ...
    print("end process rmfd dataset !!!")